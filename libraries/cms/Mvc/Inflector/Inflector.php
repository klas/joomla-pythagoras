...
namespace Joomla\Cms\Mvc\Inflector;

/**
 * An Inflector to pluralize and singularize English nouns. Unlike the Joomla\String\Inflector it will NOT try to
 * lowercase the inflected word.
 */
class Inflector
{
	/**
	...
	{
		// Handle words where the first letter is uppercase
		if (ucfirst($word) == $word)
		{
			return ucfirst($this->pluralize(strtolower($word)));
		}
		...
		// Do I have a CamelCasePhrase e.g. ControlPanel?
		$exploded = $this->explode($word);

		if (count($exploded) > 1)
		{
			$lastWord   = array_pop($exploded);
			$exploded[] = $this->pluralize($lastWord);
			$plural     = $this->implode($exploded);
	...
	public function singularize($word)
	{
		...
		{
			$lastWord   = array_pop($exploded);
			$exploded[] = $this->singularize($lastWord);
			$singular   = $this->implode($exploded);
